...
def parser():
    ...
    mlabel=mcs.add_parser('label'); mlabel.add_argument('gate',choices=GATES,nargs='?',help='Omit to label the task\'s PR_READY certification instead of one gate attempt')
    mlabel.add_argument('--task-key',required=True,help='task_key from `ai metrics --all-tasks --by task --json` or `ai current --json`\'s directory name')
    ...
    mverdict.add_argument('--true-positive',action='store_true',help='Gate label: the gate was right to fail (a real problem)')
    mverdict.add_argument('--false-positive',action='store_true',help='Gate label: the gate was wrong to fail (not a real problem)')
    mverdict.add_argument('--correct',action='store_true',help='Certification label (no gate): the final PR_READY / withheld decision matched reality')
    mverdict.add_argument('--incorrect',action='store_true',help='Certification label (no gate): the final decision was wrong -- e.g. PR_READY on a change that was not ready')
    ...
    sx=sks.add_parser('export'); sx.add_argument('--out',required=True,help='Directory to write standalone SKILL.md files into (e.g. a project\'s .claude/skills/)'); sx.add_argument('--skill',action='append',default=None,help='Export only this skill (repeatable); default: every enabled, selectable skill')
