...
'''
        ...
        if args.name in bundle:
            # No summary-file hash here: 'summary' is itself always a bundle member (it and
            # provenance are unconditionally required together, core.GATES[:7]), generated by
            ...
            if saved.get('key')==key and args.name in saved.get('verdicts',{}):
                ...
                summary_note=('\nThe summary_markdown you return under the \'summary\' key IS the generated PR '
                              'summary the other bundled gates -- provenance especially -- must judge; nothing '
